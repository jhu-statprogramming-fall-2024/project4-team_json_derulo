...
# Function to fetch route details - MOVED UP before server function
...
server <- function(input, output, session) {
  ...
  # Function to clear and update map
  update_map_display <- function(map_data, route_results = NULL) {
    ...
    if (!is.null(route_results)) {
      if (!is.null(route_results$driving)) {
        coords <- decode_pl(route_results$driving$polyline)
        google_map_update(map_id = "map") %>%
          add_polylines(
            data = coords,
            lat = "lat",
      ...
      if (!is.null(route_results$walking)) {
        coords <- decode_pl(route_results$walking$polyline)
        google_map_update(map_id = "map") %>%
          add_polylines(
            data = coords,
            lat = "lat",
      ...
      if (!is.null(route_results$transit)) {
        coords <- decode_pl(route_results$transit$polyline)
        google_map_update(map_id = "map") %>%
          add_polylines(
            data = coords,
            lat = "lat",
  ...
  observeEvent(input$google_map_marker_click, {
    ...
    if (!is.null(clicked_clinic)) {
      ...
      # Update map display
      update_map_display(rv$initial_map_data, rv$route_results)
      
      # Update route details text
      output$routeDetailsText <- renderUI({
        HTML(paste(
          "<h5>Routes to", clicked_clinic$name, "</h5>",
          "<h6>Driving Route:</h6>",
          ifelse(!is.null(rv$route_results$driving),
                 paste("Distance:", rv$route_results$driving$distance,
                       "<br>Duration:", rv$route_results$driving$duration),
                 "No driving route available"),
          "<h6>Walking Route:</h6>",
          ifelse(!is.null(rv$route_results$walking),
                 paste("Distance:", rv$route_results$walking$distance,
                       "<br>Duration:", rv$route_results$walking$duration),
                 "No walking route available"),
          "<h6>Transit Route:</h6>",
          ifelse(!is.null(rv$route_results$transit),
                 paste("Distance:", rv$route_results$transit$distance,
                       "<br>Duration:", rv$route_results$transit$duration),
  ...
  observeEvent(input$clinicTable_rows_selected, {
    ...
    output$routeDetailsText <- renderUI({
      HTML(paste(
        "<h5>Routes to", selected_clinic$name, "</h5>",
        "<h6>Driving Route:</h6>",
        ifelse(!is.null(rv$route_results$driving),
               paste("Distance:", rv$route_results$driving$distance,
                     "<br>Duration:", rv$route_results$driving$duration),
