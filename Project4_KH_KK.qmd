...
create_state_law_info <- function(state) {
  state_laws <- list(
    "Virginia" = list(
      gestational_limit = "Gestational Limit: 26 weeks, 6 days from the first day of your last menstrual period.\n",
      exceptions = "Exceptions: After 26 weeks, abortion is permitted to save the pregnant person's life and to preserve the pregnant person's general health (can include mental health).\n",
      parental_consent = "Parental Consent Required for Minors: YES; parents must be notified of decision 24 hours prior to abortion.\n",
      waiting_period_or_counseling = "Waiting Period or Counseling Required: NO",
      source = "Source: https://www.plannedparenthood.org/planned-parenthood-virginia-league/abortion-services/ab-faq "
    ),
    "Maryland" = list(
      gestational_limit = "Gestational Limit: Abortion legal throughout pregnancy.\n",
      parental_consent = "Parental Consent Required for Minors: NO; however, parent or guardian must be notified before a minor has an abortion, exceptions allowed.\n",
      waiting_period_or_counseling = "Waiting Period or Counseling Required: NO",
      source = "Source: https://www.plannedparenthood.org/planned-parenthood-maryland/client-resources/maryland-minor-laws"
    ),
    "North Carolina" = list(
      gestational_limit = "Gestational Limit: 12 weeks from the first day of last menstrual period.\n",
      exceptions = "Exceptions: Abortion allowed after through week 20 in cases of rape or incest. Abortion for life-limiting fetal anomaly  allowed through week 24. If the patient's life is in danger or there is a severe health risk, abortion allowed at any point.\n",
      parental_consent = "Parental Consent Required for Minors: YES\n",
      waiting_period_or_counseling = "Waiting Period or Counseling: YES; in-person counseling at least three days prior to abortion procedure and follow-up appointment for medication abortion patients 1-2 weeks after the abortion.",
      source = "Source: https://www.plannedparenthood.org/planned-parenthood-south-atlantic/patients/health-care-services/abortion-services/nc-abortion-faqs"
    ),
    "West Virginia" = list(
    ...
    "Washington DC" = list(
      gestational_limit = "Gestational Limit: Abortion legal throughout pregnancy.\n",
      parental_consent = "Parental Consent Required for Minors: NO\n",
      waiting_period_or_counseling = "Waiting Period or Counseling Required: NO"
    )
  )
}
...
abortion_funds <- list(
  ...
  "Maryland" = list(
    ...
    list(
      name = "Maryland Abortion Access Fund",
      website = "https://www.marylandabortionaccessfund.org",
      phone = "443-297-9893",
      description = "Provides financial assistance to Maryland residents seeking abortion care.",
      languages = "English",
      hours = "Online form available 24/7"
    )
  ),
  "Virginia" = list(
    list(
...
isolate({
  ...
  output$stateLawInfo <- renderUI({
    ...
    state_laws <- create_state_law_info()
    selected_state_laws <- state_laws[[input$lawState]]
    
    if (is.null(selected_state_laws)) {
      return(h4("No information available for this state"))
    }
    
    # Create HTML content from state laws
    if (!is.null(selected_state_laws$text)) {
      # For states where abortion is illegal
      HTML(paste0("<div class='alert alert-warning'>", selected_state_laws$text, "</div>"))
    } else {
      # For states with legal abortion
      HTML(paste0(
        "<div class='well'>",
        "<p>", selected_state_laws$gestational_limit, "</p>",
        if (!is.null(selected_state_laws$exceptions)) 
          paste0("<p>", selected_state_laws$exceptions, "</p>") else "",
        "<p>", selected_state_laws$parental_consent, "</p>",
        "<p>", selected_state_laws$waiting_period_or_counseling, "</p>",
        "<p>", selected_state_laws$source, "</p>",
        "</div>"
      ))
    }
  })
