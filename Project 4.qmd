...
# Modify the UI to replace the verbatimTextOutput
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      DTOutput("clinicTable")  # Replace verbatimTextOutput with DTOutput
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$find, {
    ...
    tryCatch({
      ...
      # Set max gestational age, use Inf if not specified
      max_gaw <- ifelse(is.na(input$max_gaw), Inf, as.numeric(input$max_gaw))
      
      # Filter clinics based on gestational age
      filtered_clinics <- clinics[sapply(clinics, function(clinic) clinic$max_GAw >= max_gaw)]
      
      # Calculate distances
      distances <- sapply(filtered_clinics, function(clinic) {
        distm(
          c(loc$lng, loc$lat), 
          c(clinic$long, clinic$lat), 
          fun = distHaversine
      ...
      clinic_data <- data.frame(
        Name = unlist(lapply(sorted_clinics, function(clinic) {
          # Create a hyperlink if website exists, otherwise just the name
          if (!is.null(clinic$website) && clinic$website != "") {
            paste0('<a href="', clinic$website, '" target="_blank">', clinic$name, '</a>')
          } else {
            clinic$name
          }
        })),
        Address = unlist(lapply(sorted_clinics, `[[`, "address")),
        Distance = round(sorted_distances, 1),
        Phone = unlist(lapply(sorted_clinics, `[[`, "phone")),
        MaxGA = unlist(lapply(sorted_clinics, `[[`, "max_GAw")),
      ...
      map_data <- rbind(
        data.frame(
          Latitude = unlist(lapply(sorted_clinics, `[[`, "lat")),
          Longitude = unlist(lapply(sorted_clinics, `[[`, "long")),
          Name = unlist(lapply(sorted_clinics, `[[`, "name")),
          colour = "red",
          stringsAsFactors = FALSE
        ),
        data.frame(
      ...
      output$clinicTable <- renderDT({
        datatable(
          clinic_data, 
          escape = FALSE,  # Allow HTML in Name column
          options = list(
            pageLength = 5,  # Show 5 entries per page
            lengthMenu = c(5, 10, 15),  # Options for number of entries
